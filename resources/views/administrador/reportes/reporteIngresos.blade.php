...
<body>
    <div class="report-wrapper">
        ...
        <div class="report-details" style="margin-bottom: 20px;">
            ...
            <p><b>Periodo de Fechas:</b> {{ $fecha_inicio ?? 'N/A' }} al {{ $fecha_final ?? 'N/A' }}</p>
        ...
        <table>
            <thead>
                @if ($usuarios_seleccionados->isNotEmpty())
                    <tr style="background-color: #eef; font-weight: 300; text-transform: uppercase">
                        <td colspan="8">
                            Boletas Registradas Por:
                            @foreach ($usuarios_seleccionados as $u)
                                {{ ucwords($u->nombres) }} {{ ucwords($u->apellidos) }}@if (!$loop->last)
                                    ,
                                @endif
                            @endforeach
                        </td>
